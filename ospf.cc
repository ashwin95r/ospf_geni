...
void *send_hello(void *useless)
{
	...
	//	cout << "sayHello" << endl;
		...
		//	cout << "SENT : " << send_data << " " << i->first << endl;
			...
			int gen_time=(cur_time.tv_sec)%100000 ;
       		router_map.time_mon[i->first] = gen_time - 3;
    ...
    while(1)
	{
		...
		if(bytes_read > 0)	
		{
    		...
    		else if(type == 2)	// HELLO REPLY packet
    		{
    			...
    			token = split(recv_data, ' ', token);
    			gettimeofday(&cur_time, NULL);
				int gen_time=cur_time.tv_sec%100000 + (cur_time.tv_usec)%100/70;
				cout << gen_time << " " << router_map.time_mon[stoi(token[1])] << endl;
