...
class Router{
	...
	public:
		...
		void setup_socket()
		{
			host = (struct hostent *) gethostbyname(("node-"+to_string(id)).c_str());
			cout << ("node-"+to_string(id)).c_str() << endl;
...
void set_receiver(int port, string ip = "localhost")
{
	struct hostent *host ;
...
void *run_djikstra(void *useless)
{
	while(1)
	{
		...
		vector<int> dist(n_routers, 10000);
